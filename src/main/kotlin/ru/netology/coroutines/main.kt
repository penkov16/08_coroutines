...
import ru.netology.coroutines.dto.Authors
        ...
        launch {
            try {
                val posts = getPosts(client)
                    .map { post ->
                        PostWithComments(post, getAuthors(client, post.id))
                    }
                println(posts)
            } catch (e: Exception) {
                e.printStackTrace()
...
suspend fun getAuthors(client: OkHttpClient, id: Long): List<Authors> =
    makeRequest("$BASE_URL/api/slow/authors/$id", client, object : TypeToken<List<Authors>>() {})
        ...
        launch {
            try {
                val posts = getPosts(client)
                    .map { post ->
                        async {
                            PostWithComments(post, getAuthors(client, post.authorId))
                        }
                    }.awaitAll()
                println(posts)
            } catch (e: Exception) {
